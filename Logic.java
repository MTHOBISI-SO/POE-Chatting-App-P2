...
public class Logic {
    ...
    public Result registerUser(String username, String phone, String password) {
        if (!username.matches(USERNAME_REGEX)) {
            return new Result(false, "Please ensure that your Username contains an underscore and is no more than five characters in length.");
        }
        ...
        if (!phone.matches(PHONE_REGEX)) {
            return new Result(false, "Phone number must be valid (local or +27 format).");
        }
